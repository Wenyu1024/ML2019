...
generate a table that list all the conditions
```{r}
...
#what is the performance? Lets organize the loss as sum of number of misclassfication cases 
...
lets store a cond_df1 first to make life easier
```{r}
  cond1 <- train %>% select(1,3) %>% distinct() %>% mutate(x_var = "x1") %>% rename(x_value=x1)
  cond2 <- train %>% select(2,3) %>% distinct() %>% mutate(x_var = "x2") %>% rename(x_value=x2)
  #note you can't use this to genrate a full condition since the sample drawn randomly from the distribution may not contain some specifc value. 
  cond_df1 <- rbind(cond1,cond2)%>% select(x_var,x_value,y)
```
...
```
I don't understand why increasing the training sample size failed to increase the test accuracy,
in my case the training and test data did come from the same distribution.

wait, it may be that by chance the test dataset are not a good representative of the underlying distribution for the population.

so now lets have multiple simulations but within each batch of simulation lets generate a new test dataset. 
```{r}
