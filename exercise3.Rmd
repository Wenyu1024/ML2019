...
for (sim in 1:sim_no){
  ...
  for (ii in 1:9){
    ...
    px1 <- table(X[,1])/samplesize
    px2 <- table(X[,2])/samplesize
    #note that as an extreme of poorly distributed sample, if training sample size are too small, certain classes may not even exist the training sample.
    if (length(px1) <2) {
      tmp <- names(px1)
      tmp1 <- c("0","1")[!(c("0","1") %in% tmp)]
      tmp2 <- 0.0000001
      names(tmp2) <- tmp1
      px1 <- c(px1, tmp2 )
    }
    ...
    for (i in 1:6){
      ...
      for (a in 1: 6) {
        ...
        # why is there no p values? sample are too small and class are not well represented in the training sample.
        # if (length(c(0,1,2)[which.max(p)]) != 0){
        #   y_pred <- c(0,1,2)[which.max(p)]
        # } else {
        #   y_pred <- c(0,1,2)[which.max(p)]
        # }
    ...
    logi_m <- nnet::multinom(formula = (y ~x1+x2),data = data_logistic)
